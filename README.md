...
Version `0.0.11` ships the real product foundation with the approved fleet shell polish:
...
The container serves the full app from the API process, so self-hosting is a single container in `0.0.11`.
...
```bash
DOCKER_IMAGE=deepdaddyttv/bambuview:0.0.11 pnpm deploy:portainer
```
...
- Desktop installers are intentionally deferred beyond `0.0.11`
