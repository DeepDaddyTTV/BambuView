...
Version `0.0.3` ships the real product foundation:
...
The container serves the full app from the API process, so self-hosting is a single container in `0.0.3`.
...
```bash
DOCKER_IMAGE=deepdaddyttv/bambuview:0.0.3 pnpm deploy:portainer
```
...
- Desktop installers are intentionally deferred beyond `0.0.3`
