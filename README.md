...
Version `0.0.19` keeps the container-first revision workflow and tightens the Fleet shell toward the approved graphite-and-green mockup with centered official SVG branding, centered sidebar rail cards, and a straight-edged UI pass:
...
The container serves the full app from the API process, so self-hosting is a single container in `0.0.19`.
...
```bash
DOCKER_IMAGE=deepdaddyttv/bambuview:0.0.19 pnpm deploy:portainer
```
...
- Desktop installers are intentionally deferred beyond `0.0.19`
