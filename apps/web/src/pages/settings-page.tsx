...
export function SettingsPage() {
  ...
  const previewQuery = useQuery({
    queryKey: ["fleet-overview", "placeholder", "settings-preview"],
    queryFn: () =>
      apiFetch<FleetOverview>("/api/fleet/overview?mode=placeholder"),
  });
      ...
      <div className="space-y-6">
        ...
        <section className="panel">
          ...
          <p className="mt-3 text-sm leading-7 text-zinc-400">
            Choose how the approved graphite shell picks up your selected
            highlight color.
          </p>
          <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {backgroundOptions.map((option) => (
              ...
              >
                <div
                  className={`background-card__preview app-background app-background--${option.key}`}
                />
                <div className="mt-3 text-left text-base font-medium text-white">
                  {option.label}
                </div>
              </button>
            ))}
          ...
          <div className="mt-5 flex items-center gap-3 text-sm text-zinc-400">
            ...
            {errorMessage ??
              (isSaving
                ? "Saving appearance…"
                : "Appearance is synced to your local account.")}
          </div>
        </section>
      </div>
      <section className="panel">
        ...
        <p className="mt-3 text-sm leading-7 text-zinc-400">
          The preview reflects your actual appearance settings while keeping the
          approved fleet layout intact.
        </p>
        <div className="mt-5">
